  ...
  return (
    <div className="flex h-dvh md:h-auto overflow-hidden md:overflow-visible">
      ...
      <div className="flex-1 min-w-0 flex flex-col h-full md:h-auto overflow-hidden md:overflow-visible">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="border-l border-brand-mid pl-3 min-w-0">
              <p className="text-xs md:text-sm text-brand-light flex items-center gap-1.5 truncate">
                <span>🔒</span>
                <span className="truncate">Track where your pennies roll while your data stays on your device</span>
              </p>
            </div>
          </div>
        </header>
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-brand-dark md:hidden z-30 flex">
          <button
            onClick={handleSelectDashboard}
            className={`flex-1 py-3 flex flex-col items-center gap-0.5 transition-colors ${
              activeView === 'dashboard' ? 'text-brand-accent' : 'text-brand-light/70'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-3 flex flex-col items-center gap-0.5 transition-colors ${
              activeView === 'month' ? 'text-brand-accent' : 'text-brand-light/70'
            }`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
